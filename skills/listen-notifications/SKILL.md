...
Use this skill to send a notification to a human through the `listen` CLI. Configured browser subscriptions may deliver it as a system push notification. Assume `listen` is already installed, available on `PATH` or in `./.bin/listen`, and configured with a webhook URL.
